...
// ResolvePasetoKey picks the PASETO signer material. An explicit
// PASETO_PRIVATE_KEY always wins; otherwise MNEMONIC is derived.
func ResolvePasetoKey(explicitHex, mnemonic string) (key string, fromMnemonic bool, err error) {
	explicitHex = strings.TrimSpace(explicitHex)
	if explicitHex != "" {
		return explicitHex, false, nil
	}
	mnemonic = strings.TrimSpace(mnemonic)
	if mnemonic == "" {
		return "", false, nil
	}
	key, err = PasetoKeyFromMnemonic(mnemonic)
	if err != nil {
		return "", false, err
	}
	return key, true, nil
