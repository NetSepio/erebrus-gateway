...
// GetUserPlan returns the user's effective plan: org-paid plan first, then active subscription.
func (s *Store) GetUserPlan(ctx context.Context, userID string) (string, error) {
	if orgPlan, err := s.UserOrgVPNPlan(ctx, userID); err == nil && orgPlan != "" {
		return orgPlan, nil
	}
...
// SetUserPlan creates or replaces an admin-sourced subscription for a user.
func (s *Store) SetUserPlan(ctx context.Context, userID, planID string) (*Subscription, error) {
	var sub Subscription
	end := time.Now().AddDate(1, 0, 0)
	// Deactivate any existing user subscription and create a new admin row.
	err := s.db.QueryRowContext(ctx,
		`WITH inactive AS (
		 ...
		 SELECT id, plan_id, source, status, current_period_end, created_at FROM inserted`,
		userID, planID, end).
		Scan(&sub.ID, &sub.PlanID, &sub.Source, &sub.Status, &sub.CurrentPeriodEnd, &sub.CreatedAt)
	return &sub, err
}

// CountLegacyActiveSubscriptionsByPlan returns active subscription counts per plan (admin).
